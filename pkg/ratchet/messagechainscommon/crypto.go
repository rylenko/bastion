...
const cryptoHeaderCipherKDFOutputLen = cipher.KeySize + cipher.NonceSizeX

var (
	cryptoHeaderCipherKDFInfo = []byte("header")
	cryptoHeaderCipherKDFSalt = make([]byte, cryptoHeaderCipherKDFOutputLen)
)
...
func DeriveHeaderCipherKeyAndNonce(headerKey keys.Header, messageNumber uint64) ([]byte, []byte, error) {
	...
	kdfKey := buildHeaderCipherKDFKey(headerKey, messageNumber)
	kdf := hkdf.New(func() hash.Hash { return hasher }, kdfKey, cryptoHeaderCipherKDFSalt, cryptoHeaderCipherKDFInfo)

	output := make([]byte, cryptoHeaderCipherKDFOutputLen)
...
// buildHeaderCipherKDFKey builds KDF key to derive cipher key and nonce.
//
...
func buildHeaderCipherKDFKey(headerKey keys.Header, messageNumber uint64) []byte {
	var messageNumberBytes [utils.Uint64Size]byte
	binary.LittleEndian.PutUint64(messageNumberBytes[:], messageNumber)

	return utils.ConcatByteSlices(messageNumberBytes[:], headerKey.Bytes)
}
